        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-slate-900">Active Pipeline Alerts</h3>
              <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-slate-200/70 text-slate-600">
                {isExpanded ? 'Click to collapse' : 'Click to expand'}
              </span>
            </div>
            <p className="text-xs text-slate-500">Real-time task failures and data freshness anomalies</p>
          </div>
        </div>
          ...
          {isAlertsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-xs text-slate-500 max-w-md mt-0.5">
                The pipeline is currently operating normally with zero active errors or unhandled lag alerts.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100 -my-3">
              {alerts.map((alert: AlertEvent) => (
                <div key={alert.alert_id} className="py-3.5 first:pt-0 last:pb-0">
